...
class Conversation:
    ...
    def _load_conversation(
        ...
        self.load_conversations(
            definition.get("conversations", {}), settings=self.settings
        )
        ...
        for template in templates:
            ...
            with open(template, "r", encoding="utf-8") as stream:
                try:
                    ...
                    log.error(
                        f"Error while reading: {template}, definitions being ignored"
                    )
                    log.error(exec)
                    sys.exit()
        ...
        for prompts_ in prompts:
            ...
            with open(prompts_, "r", encoding="utf-8") as stream:
                try:
                    ...
                    log.error(
                        f"Error while reading: {prompts}, definitions being ignored [{exc}]"
                    )
                    log.error(exec)
                    sys.exit()
    ...
    def load_conversations(self, conversations_: dict, settings: dict = {}):
        ...
        for filename in conversations_:
            ...
            if basename not in self.conversations:
                ...
                self.conversations[basename] = {
                    "project_pathname": self.project_pathname,
                    "filename": filename,
                    "user_id": self.user_id,
                }
            else:
                log.info(
                    f"Conversation '{filename}' already in {self.basename}, it will be ignored"
                )
