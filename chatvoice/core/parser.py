...
Grammar (informal):
    ...
    command    ::= "if" condition "then" simple_cmd | simple_cmd
    simple_cmd ::= WORD arg*
...
class Command:
    ...
    def __str__(self) -> str:
        parts = []
        if self.condition:
            parts.append(f"if {self.condition} then")
...
# A parsed line is always a Chain (even if it holds only one command).
Line = Chain
...
_OP_RE = re.compile(r"==|!=|<=|>=|<|>")
_IF_THEN_RE = re.compile(r"^if\s+(.+?)\s+then\s+(.+)$", re.IGNORECASE)


def _tokenize(text: str) -> list[str]:
    """Split *text* into tokens, keeping quoted strings together.

    Uses :mod:`shlex` so that ``"hello world"`` stays as one token and
    escape sequences inside quotes are handled correctly.
    """
    lex = shlex.shlex(text, posix=True)
    lex.whitespace_split = True          # keep operators together
    lex.whitespace = " \t"
    return list(lex)
...
def _parse_clause(text: str) -> Clause:
    ...
    # bare identifier: ``if logged_in then …``
