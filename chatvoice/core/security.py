...
async def authenticate_user(
    ...
    if "@" in username_or_email:
        db_user = await crud_users.get(
            db=db, email=username_or_email, is_deleted=False, is_verified=True
        )
    else:
        db_user = await crud_users.get(
            db=db, username=username_or_email, is_deleted=False, is_verified=True
        )
...
def _create_token(
    ...
    if expires_delta:
        ...
        if token_type not in default_deltas:
            raise ValueError(
                f"Token type '{token_type}' requires explicit expires_delta"
            )
...
def create_verification_token(
    ...
    return _create_token(
        data={"sub": email}, token_type=token_type, expires_delta=expires_delta
    )
...
def decode_verification_token(
    ...
    try:
        ...
        raise CustomException(
            status_code=401, detail="El token ha expirado. Solicita un nuevo token."
        )
    except jwt.InvalidTokenError:
        raise CustomException(
            status_code=401, detail="El token proporcionado es incorrecto."
        )
