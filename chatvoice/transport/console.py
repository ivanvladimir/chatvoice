        ...
        with get_db_ctx() as db:
            if "@" in username_or_email:
                query = select(User).filter_by(
                    email=username_or_email, is_deleted=False, is_verified=True
                )
            else:
                query = select(User).filter_by(
                    username=username_or_email, is_deleted=False, is_verified=True
                )
            ...
            if not db_user:
                log.error(
                    "Username or email not found", username_or_email=username_or_email
                )
                return False
    ...
    def run(self, user_id: int, interpreter: Callable):
        ...
        while True:
            ...
            if m["cmd"] == "say" and len(m["args"]) > 0:
                for msg in m["args"]:
                    self.console.print(
                        f"[blue]{interpreter.settings['_name_system']}[/]:", end=" "
                    )
                    self.console.print(Markdown(msg))
                    self.console.file.flush()
            elif m["cmd"] == "listen":
                input = self.console.input(
                    f"[red]{interpreter.settings['_name_user']}[/]: "
                )
                session.send(input)
                self.console.print()
                self.console.file.flush()
