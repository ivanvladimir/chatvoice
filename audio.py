...
def tts_google(msg,lang='es-us'):
    ...
    p = Popen(['mpg321','-w',mp3_filename.replace('.mp3','.wav'), mp3_filename], stdout=DEVNULL, stderr=STDOUT)
    p.communicate()
    assert p.returncode == 0
    p = Popen(['sudo','aplay',mp3_filename.replace(".mp3",".wav"),"-D",'default:CARD=Headset'])
    p.communicate()
    assert p.returncode == 0
