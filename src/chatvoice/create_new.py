...
if __name__ == "__main__":
    ...
    p.add_argument("CONVDIR", nargs="?", help="Conversation file")
    p.add_argument(
        "-n",
        "--name",
        default="CHATVOICE",
    ...
    p.add_argument(
        "-v", "--verbose", action="store_true", help="Verbose mode [%(default)s]"
    )
...
"""
    ...
    with open(
        os.path.join(args.path, args.CONVDIR, args.main), "w", encoding="utf-8"
    ) as f:
        f.write(main)
