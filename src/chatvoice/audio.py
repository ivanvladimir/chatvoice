...
# 3 - listening voice
# 4 - stop listening
STATE = {"main": 0, "time_start_listening": None}
...
def callback(indata__, frames, time, status):
    ...
    for i in range(10):
        is_speech = vad.is_speech(
            audio_data[i * FRAMES_PER_BUFFER : i * FRAMES_PER_BUFFER + 160].tobytes(),
            SAMPLERATE,
        )
    ...
    if STATE_ == 2:
        ...
        if num_voiced > 0.25 * NUM_WINDOW_CHUNKS:
            ...
            filename_wav = os.path.join(
                SPEECHRECDIR, f'{datetime.now().strftime("%Y%m%d_%H%M%S")}.wav'
            )
